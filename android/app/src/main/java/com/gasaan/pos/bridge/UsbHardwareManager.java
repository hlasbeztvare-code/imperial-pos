...
public class UsbHardwareManager {
    ...
    private boolean tryConnect(UsbDevice device) {
        // Fix 2: Strictly filter by USB Printer Class to avoid grabbing touch controllers or scanners
        ...
        if (usbInterface == null) {
            Log.d(TAG, "Device " + device.getDeviceName() + " is not a printer class device. Skipping.");
            return false;
        }
        ...
        for (int i = 0; i < usbInterface.getEndpointCount(); i++) {
            ...
            if (endpoint.getType() == UsbConstants.USB_ENDPOINT_XFER_BULK &&
                ...
                if (mConnection != null && mConnection.claimInterface(usbInterface, true)) {
                    ...
                    Log.i(TAG, "Connected to printer: " + device.getDeviceName());
                    return true;
                }
            }
        }
    ...
    public void sendData(byte[] data) throws Exception {
        ...
        // Fix 4: Chunk data to avoid buffer overflow on Android 7 bulk transfer (16KB limit)
        int chunkSize = 4096;
        int totalSent = 0;
        
        for (int i = 0; i < data.length; i += chunkSize) {
            int length = Math.min(data.length - i, chunkSize);
            byte[] chunk = java.util.Arrays.copyOfRange(data, i, i + length);
            
            int result = mConnection.bulkTransfer(mEndpointOut, chunk, length, 5000);
            if (result < 0) {
                throw new Exception("Bulk transfer failed at offset " + i + ": " + result);
            }
            totalSent += result;
        }
        
        Log.d(TAG, "Sent total " + totalSent + " bytes to hardware");
    }
